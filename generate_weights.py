...
def generate_weight(MODEL_PATH, DEMONS_PATH, SAVE_PATH):
    ...
    for j in range(gt_data.shape[0]-2):   #gt_data.shape[0]-2 for normal panda, 18,20for disabled panda
        state = env.reset(j)
        i = 0
        done = False
        while not done:
...
if __name__ == "__main__":
    generate_weight('..\\logs\\models\\sb-trpo-joint-target-diffdynamics-2021-05-22/best_model.zip', 
                    '..\\logs\\data\\infeasible_traj_9_1_largem.pkl',
                    '..\\logs\\data\\infeasible_traj_9_1_largem_with_weights.pkl')
