...
def collect_samples(env, policy, custom_reward,
    ...
    log['avg_reward'] = total_reward / (num_episodes if num_episodes > 0 else 1)
    log['max_reward'] = max_reward
    log['min_reward'] = min_reward
    if custom_reward is not None:
        log['total_c_reward'] = total_c_reward
        log['avg_c_reward'] = total_c_reward / (num_episodes if num_episodes > 0 else 1)
        log['max_c_reward'] = max_c_reward
        log['min_c_reward'] = min_c_reward

    return memory, log

def evaluate_policy(env, policy, custom_reward,
                    mean_action, render, running_state, update, trajectory, dtype=torch.float32):
    log = dict()
    memory = OnlineMemory()
    num_steps = 0
    total_reward = 0
    ...
    for iiid in range(100):
        ...
        for t in range(10000):
            ...
            if 'reward_eval' in infos:
                reward_episode += infos['reward_eval']
            else:
                reward_episode += reward
...
class Agent:
    ...
    def collect_samples(self, min_batch_size, trajectory=None):
        ...
        memory, log = collect_samples(self.env, self.policy, None, self.mean_action,
                                      self.render, self.running_state, min_batch_size, self.update, trajectory, self.dtype)
    ...
    def evaluate_policy(self, trajectory=None):
        self.env.seed(2333)
        to_device(torch.device('cpu'), self.policy)
        memory, log = evaluate_policy(self.env, self.policy, None, self.mean_action,
                                      self.render, self.running_state, self.update, trajectory, self.dtype)
