...
def get_args():
    ...
    parser.add_argument('--expert-traj-paths', metavar='G', nargs='+',
    ...
    parser.add_argument('--exp_id', default='mujoco', type='what experiment to run')
    args = parser.parse_args()
    return args
# load trajectory

def load_demos(demo_file, ratio):
    traj = pickle.load(open(expert_traj_path, "rb"))
    traj = traj['obs'][0:int(len(traj['obs'])*ratio)]
    return [np.array(item) for item in traj]
...
if __name__ == '__main__':
    ...
    expert_traj_raw = []
    for i in range(len(args.expert_traj_paths)):
        expert_traj_path = args.expert_traj_paths[i]
        #expert_traj_raw += pickle.load(open(expert_traj_path, "rb")) # list of expert trajectories
        if args.exp_id == 'mujoco':
            expert_traj_raw += load_demos(expert_traj_path, args.ratios[i])
        elif args.exp_id == 'simulated':
            expert_traj_raw += list(pickle.load(open(expert_traj_path, 'rb')))
